...
from calibpy.utils.helpers import export_features, getObjectPointsNNSingle, polyfit
...
def run_calibration(image_folders: Union[str, Path]):
    ...
    cam_features = Path("calibpy/data/cam_feature_data.pkl")
    ...
    selected_cam_feature_data = [
        cam_feature_data[str(i)] for i in range(10)
    ]  # !!! Must only start from 0
    ...
    projector_flags = cv2.CALIB_FIX_ASPECT_RATIO
    ...
    output_file = "calibpy/tests/stereo_calib_result.json"
...
if __name__ == "__main__":
    run_calibration("calibpy/tests/test_images/")
